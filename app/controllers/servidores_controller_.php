...
class ServidoresController extends AppController {
    ...
    function index() {
        ...
        if (!empty($this->data)) {
            ...
            if ($this->MathCaptcha->validates($this->data['Servidor']['security_code'])) {
                if ($this->Servidor->validates()) {
                    
                }
            } else {
                $this->Session->setFlash(__('Por favor, informe a resposta correta para a questão.', true));
            ...
            $servidor = $this->Servidor->DadosServidor->find('first', array('conditions' =>
                        ...
                        //'DadosServidor.CPF' => $this->data['Servidor']['cpf'],
                        )
                            )
            );
            if (!$servidor) {
